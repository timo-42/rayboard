...
func TestOpenAPIJSON(t *testing.T) {
	...
	assertRequestBodyFields(t, spec, "/api/projects/{project_id}/labels", http.MethodPost, []string{"spec"}, []string{"spec", "label"}, []string{"spec", "description"}, []string{"spec", "color"})
	assertRequestBodyFields(t, spec, "/api/projects/{project_id}/labels/{label}", http.MethodPatch, []string{"spec"}, []string{"spec", "description"}, []string{"spec", "color"})
	assertResponseBodyFields(t, spec, "/api/projects/{project_id}/labels", http.MethodGet, "200", []string{"metadata"}, []string{"metadata", "count"}, []string{"status"}, []string{"status", "items"}, []string{"status", "items", "metadata"}, []string{"status", "items", "metadata", "project_id"}, []string{"status", "items", "spec"}, []string{"status", "items", "spec", "label"}, []string{"status", "items", "spec", "description"}, []string{"status", "items", "spec", "color"}, []string{"status", "items", "status"}, []string{"status", "items", "status", "ticket_count"})
	assertResponseBodyFields(t, spec, "/api/projects/{project_id}/labels", http.MethodPost, "201", []string{"metadata"}, []string{"metadata", "id"}, []string{"metadata", "project_id"}, []string{"spec"}, []string{"spec", "label"}, []string{"status"}, []string{"status", "ticket_count"})
	assertResponseBodyFields(t, spec, "/api/projects/{project_id}/labels/{label}", http.MethodPatch, "200", []string{"metadata"}, []string{"metadata", "id"}, []string{"metadata", "project_id"}, []string{"spec"}, []string{"spec", "label"}, []string{"status"}, []string{"status", "ticket_count"})
	...
	assertResponseBodyFields(t, spec, "/api/versions/{version_id}/report", http.MethodGet, "200", []string{"metadata"}, []string{"metadata", "id"}, []string{"spec"}, []string{"spec", "version"}, []string{"status"}, []string{"status", "scope"}, []string{"status", "snapshot_at"}, []string{"status", "progress"}, []string{"status", "tickets"})
	...
	assertRequestBodyFields(t, spec, "/api/search", http.MethodPost, []string{"spec"}, []string{"spec", "text"}, []string{"spec", "sort"}, []string{"spec", "limit"}, []string{"spec", "cursor"})
	assertResponseBodyFields(t, spec, "/api/search", http.MethodPost, "200", []string{"metadata"}, []string{"spec"}, []string{"status"}, []string{"status", "items"}, []string{"status", "items", "metadata"}, []string{"status", "items", "spec"}, []string{"status", "items", "status"}, []string{"status", "next_cursor"})
	...
	assertRequestBodyFields(t, spec, "/api/notification-hooks", http.MethodPost, []string{"spec"}, []string{"spec", "name"}, []string{"spec", "actor_user_id"}, []string{"spec", "event_types"}, []string{"spec", "engine"})
	assertDiscriminatedEngineSchema(t, spec, requestBodySchema(t, spec, "/api/notification-hooks", http.MethodPost), []string{"spec", "engine"})
	assertResponseBodyFields(t, spec, "/api/notification-hooks", http.MethodGet, "200", []string{"metadata"}, []string{"metadata", "count"}, []string{"spec"}, []string{"status"}, []string{"status", "items"}, []string{"status", "items", "metadata"}, []string{"status", "items", "metadata", "scope_type"}, []string{"status", "items", "spec"}, []string{"status", "items", "spec", "engine"}, []string{"status", "items", "status"})
	assertResponseBodyFields(t, spec, "/api/notification-hooks", http.MethodPost, "201", []string{"metadata"}, []string{"metadata", "id"}, []string{"metadata", "scope_type"}, []string{"spec"}, []string{"spec", "event_types"}, []string{"spec", "engine"}, []string{"status"}, []string{"status", "last_error"})
	assertRequestBodyFields(t, spec, "/api/projects/{project_id}/notification-hooks", http.MethodPost, []string{"spec"}, []string{"spec", "name"}, []string{"spec", "actor_user_id"}, []string{"spec", "event_types"}, []string{"spec", "engine"})
	assertResponseBodyFields(t, spec, "/api/notification-hooks/{hook_id}", http.MethodGet, "200", []string{"metadata"}, []string{"metadata", "id"}, []string{"metadata", "scope_type"}, []string{"spec"}, []string{"spec", "name"}, []string{"spec", "engine"}, []string{"status"}, []string{"status", "last_error"})
	assertRequestBodyFields(t, spec, "/api/notification-hooks/{hook_id}", http.MethodPatch, []string{"spec"}, []string{"spec", "enabled"}, []string{"spec", "engine"})
	assertRequestBodyFields(t, spec, "/api/notification-hooks/{hook_id}/preview", http.MethodPost, []string{"spec"}, []string{"spec", "event_type"}, []string{"spec", "payload"}, []string{"spec", "destination_ids"})
	assertResponseBodyFields(t, spec, "/api/notification-hooks/{hook_id}/preview", http.MethodPost, "200", []string{"metadata"}, []string{"metadata", "hook_id"}, []string{"metadata", "run_id"}, []string{"spec"}, []string{"status"}, []string{"status", "state"}, []string{"status", "output"}, []string{"status", "plan"}, []string{"status", "plan", "destination_ids"})
	assertResponseBodyFields(t, spec, "/api/notification-hooks/{hook_id}/runs", http.MethodGet, "200", []string{"metadata"}, []string{"spec"}, []string{"status"}, []string{"status", "items"}, []string{"status", "items", "metadata"}, []string{"status", "items", "spec"}, []string{"status", "items", "status"})
	...
	assertResponseBodyFields(t, spec, "/api/settings", http.MethodGet, "200", []string{"metadata"}, []string{"metadata", "id"}, []string{"spec"}, []string{"spec", "attachment_max_size_bytes"}, []string{"spec", "webhook_allowed_base_urls"}, []string{"status"}, []string{"status", "attachment_policy_active"})
	...
	assertRequestBodyFields(t, spec, "/api/engines/test", http.MethodPost, []string{"spec"}, []string{"spec", "surface"}, []string{"spec", "engine"}, []string{"spec", "context"}, []string{"spec", "input"}, []string{"spec", "dry_run"}, []string{"spec", "validate_only"})
	...
	assertResponseBodyFields(t, spec, "/api/engines/test", http.MethodPost, "200", []string{"metadata"}, []string{"metadata", "id"}, []string{"spec"}, []string{"spec", "engine"}, []string{"spec", "context"}, []string{"spec", "dry_run"}, []string{"spec", "validate_only"}, []string{"status"}, []string{"status", "state"}, []string{"status", "mode"}, []string{"status", "output"}, []string{"status", "logs"}, []string{"status", "duration_millis"}, []string{"status", "engine"}, []string{"status", "error"})
	...
	assertRequestBodyFields(t, spec, "/api/webhook-definitions/{webhook_id}", http.MethodPatch, []string{"spec"}, []string{"spec", "name"}, []string{"spec", "direction"}, []string{"spec", "enabled"}, []string{"spec", "actor_user_id"}, []string{"spec", "event_types"}, []string{"spec", "engine"})
	...
	assertResponseBodyFields(t, spec, "/api/webhook-definitions/{webhook_id}/deliveries", http.MethodGet, "200", []string{"metadata"}, []string{"metadata", "count"}, []string{"spec"}, []string{"status"}, []string{"status", "items"}, []string{"status", "items", "metadata"}, []string{"status", "items", "metadata", "webhook_id"}, []string{"status", "items", "spec"}, []string{"status", "items", "spec", "event_type"}, []string{"status", "items", "status"}, []string{"status", "items", "status", "state"})
	assertResponseBodyFields(t, spec, "/api/webhook-deliveries/{delivery_id}", http.MethodGet, "200", []string{"metadata"}, []string{"metadata", "webhook_id"}, []string{"metadata", "domain_event_id"}, []string{"spec"}, []string{"spec", "event_type"}, []string{"spec", "payload"}, []string{"status"}, []string{"status", "state"}, []string{"status", "attempt_count"})
	...
	assertResponseBodyFields(t, spec, "/api/ticket-hooks/{hook_id}/runs", http.MethodGet, "200", []string{"metadata"}, []string{"metadata", "count"}, []string{"spec"}, []string{"status"}, []string{"status", "items"}, []string{"status", "items", "metadata"}, []string{"status", "items", "spec"}, []string{"status", "items", "status"}, []string{"status", "items", "status", "state"})
	...
	assertResponseBodyFields(t, spec, "/api/ticket-create-pages/{page_id}/preview", http.MethodPost, "200", []string{"metadata"}, []string{"metadata", "page_id"}, []string{"metadata", "project_id"}, []string{"metadata", "slug"}, []string{"spec"}, []string{"spec", "field_layout"}, []string{"spec", "defaults"}, []string{"status"}, []string{"status", "enabled"})
	assertResponseBodyOmitsFields(t, spec, "/api/ticket-create-pages/{page_id}/preview", http.MethodPost, "200", []string{"spec", "form_lua_script"}, []string{"spec", "form_ai_prompt"}, []string{"spec", "form_ai_provider_id"})
	assertResponseBodyFields(t, spec, "/api/ticket-create-pages/{page_id}/runs", http.MethodGet, "200", []string{"metadata"}, []string{"metadata", "count"}, []string{"spec"}, []string{"status"}, []string{"status", "items"}, []string{"status", "items", "metadata"}, []string{"status", "items", "spec"}, []string{"status", "items", "status"}, []string{"status", "items", "status", "state"})
	...
	assertResponseBodyFields(t, spec, "/api/projects/{project_id}/roadmap/dependencies", http.MethodGet, "200", []string{"metadata"}, []string{"metadata", "count"}, []string{"status"}, []string{"status", "items"}, []string{"status", "items", "metadata"}, []string{"status", "items", "metadata", "project_id"}, []string{"status", "items", "spec"}, []string{"status", "items", "spec", "link"}, []string{"status", "items", "spec", "source_epic_id"}, []string{"status", "items", "spec", "target_epic_id"})
	...
	assertRequestBodyFields(t, spec, "/api/tickets/{ticket_id}/links", http.MethodPost, []string{"spec"}, []string{"spec", "target_ticket_id"}, []string{"spec", "link_type"})
	assertResponseBodyFields(t, spec, "/api/tickets/{ticket_id}/links", http.MethodGet, "200", []string{"metadata"}, []string{"status"}, []string{"status", "items"}, []string{"status", "items", "metadata"}, []string{"status", "items", "metadata", "project_id"}, []string{"status", "items", "spec"}, []string{"status", "items", "spec", "link_type"}, []string{"status", "items", "spec", "source"}, []string{"status", "items", "spec", "target"}, []string{"status", "items", "status"}, []string{"status", "items", "status", "created_by"})
	assertResponseBodyFields(t, spec, "/api/tickets/{ticket_id}/links", http.MethodPost, "201", []string{"metadata"}, []string{"metadata", "project_id"}, []string{"spec"}, []string{"spec", "link_type"}, []string{"spec", "source"}, []string{"spec", "target"}, []string{"status"}, []string{"status", "created_by"})
	assertResponseBodyFields(t, spec, "/api/tickets/{ticket_id}/watchers/me", http.MethodPut, "200", []string{"metadata"}, []string{"spec"}, []string{"status"}, []string{"status", "watching"}, []string{"status", "watcher_count"})
	...
	assertResponseBodyFields(t, spec, "/api/projects/{project_id}/roadmap/capacity-targets", http.MethodGet, "200", []string{"metadata"}, []string{"metadata", "count"}, []string{"status"}, []string{"status", "items"}, []string{"status", "items", "metadata"}, []string{"status", "items", "metadata", "project_id"}, []string{"status", "items", "metadata", "month"}, []string{"status", "items", "spec"}, []string{"status", "items", "spec", "target_points"})
	assertRequestBodyFields(t, spec, "/api/projects/{project_id}/roadmap/capacity-targets/{month}", http.MethodPut, []string{"spec"}, []string{"spec", "target_points"})
	assertResponseBodyFields(t, spec, "/api/projects/{project_id}/roadmap/capacity-targets/{month}", http.MethodPut, "200", []string{"metadata"}, []string{"metadata", "project_id"}, []string{"metadata", "month"}, []string{"spec"}, []string{"spec", "target_points"}, []string{"status"}, []string{"status", "deleted"})
	assertRequestBodyFields(t, spec, "/api/projects/{project_id}/roadmap/schedule", http.MethodPatch, []string{"spec"}, []string{"spec", "ticket_id"}, []string{"spec", "start_date"}, []string{"spec", "due_date"})
	assertResponseBodyFields(t, spec, "/api/projects/{project_id}/roadmap/schedule", http.MethodPatch, "200", []string{"metadata"}, []string{"status"}, []string{"status", "items"}, []string{"status", "items", "spec"}, []string{"status", "items", "spec", "epic"})
