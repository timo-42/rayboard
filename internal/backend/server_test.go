...
func TestOpenAPIJSON(t *testing.T) {
	...
	assertRequestBodyFields(t, spec, "/api/projects/{project_id}/webhooks", http.MethodPost, []string{"spec"}, []string{"spec", "name"}, []string{"spec", "direction"}, []string{"spec", "actor_user_id"}, []string{"spec", "engine"})
	...
	assertResponseBodyFields(t, spec, "/api/projects/{project_id}/webhooks", http.MethodPost, "201", []string{"metadata"}, []string{"metadata", "project_id"}, []string{"spec"}, []string{"spec", "engine"}, []string{"status"}, []string{"status", "token_set"}, []string{"status", "token"})
	assertResponseBodyFields(t, spec, "/api/webhook-definitions/{webhook_id}", http.MethodGet, "200", []string{"metadata"}, []string{"metadata", "id"}, []string{"spec"}, []string{"spec", "name"}, []string{"status"}, []string{"status", "token_set"})
	assertRequestBodyFields(t, spec, "/api/webhook-definitions/{webhook_id}", http.MethodPatch, []string{"spec"}, []string{"spec", "enabled"})
	...
	assertResponseBodyFields(t, spec, "/api/webhooks/incoming/{webhook_id}", http.MethodPost, "200", []string{"metadata"}, []string{"spec"}, []string{"status"}, []string{"status", "token_set"})
