...
func TestOpenAPIJSON(t *testing.T) {
	...
	assertRequestBodyFields(t, spec, "/api/engines/test", http.MethodPost, []string{"spec"}, []string{"spec", "surface"}, []string{"spec", "engine"}, []string{"spec", "input"})
	assertDiscriminatedEngineSchema(t, spec, requestBodySchema(t, spec, "/api/engines/test", http.MethodPost), []string{"spec", "engine"})
	assertResponseBodyFields(t, spec, "/api/engines/test", http.MethodPost, "200", []string{"metadata"}, []string{"metadata", "id"}, []string{"spec"}, []string{"spec", "engine"}, []string{"status"}, []string{"status", "state"}, []string{"status", "output"}, []string{"status", "error"})
