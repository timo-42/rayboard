...
func TestSprintLifecycleAndTicketAssignment(t *testing.T) {
	...
	version, err := service.CreateVersion(ctx, admin, tracker.CreateVersionInput{ProjectID: project.ID, Name: "1.0"})
	if err != nil {
		t.Fatalf("create version: %v", err)
	}
	ticketPriority := "high"
	ticketType := "bug"
	ticketStartDate := "2026-06-20"
	ticketDueDate := "2026-06-24"
	...
	ticket, err := service.CreateTicket(ctx, admin, tracker.CreateTicketInput{ProjectID: project.ID, Title: "Sprint ticket", Priority: ticketPriority, Type: ticketType, ParentTicketID: epic.ID, ComponentID: component.ID, VersionID: version.ID, StartDate: ticketStartDate, DueDate: ticketDueDate, Labels: ticketLabels})
	if err != nil {
		t.Fatalf("create ticket: %v", err)
	}
	...
	if activeReport.Scope != tracker.SprintReportScopeCurrent || activeReport.SnapshotAt != nil || activeReport.Progress.Total != 1 || activeReport.Progress.Done != 0 || len(activeReport.Tickets) != 1 || activeReport.Tickets[0].ID != ticket.ID || activeReport.Tickets[0].ReporterID != ticket.ReporterID || activeReport.Tickets[0].Priority != ticketPriority || activeReport.Tickets[0].Type != ticketType || activeReport.Tickets[0].ParentTicketID != epic.ID || activeReport.Tickets[0].ComponentID != component.ID || activeReport.Tickets[0].VersionID != version.ID || activeReport.Tickets[0].StartDate != ticketStartDate || activeReport.Tickets[0].DueDate != ticketDueDate || !slices.Equal(activeReport.Tickets[0].Labels, expectedTicketLabels) {
	...
	if completedReportAfterMove.Scope != tracker.SprintReportScopeSnapshot || completedReportAfterMove.Progress.Total != 1 || len(completedReportAfterMove.Tickets) != 1 || completedReportAfterMove.Tickets[0].ID != ticket.ID || completedReportAfterMove.Tickets[0].ReporterID != ticket.ReporterID || completedReportAfterMove.Tickets[0].Priority != ticketPriority || completedReportAfterMove.Tickets[0].Type != ticketType || completedReportAfterMove.Tickets[0].ParentTicketID != epic.ID || completedReportAfterMove.Tickets[0].ComponentID != component.ID || completedReportAfterMove.Tickets[0].VersionID != version.ID || completedReportAfterMove.Tickets[0].StartDate != ticketStartDate || completedReportAfterMove.Tickets[0].DueDate != ticketDueDate || !slices.Equal(completedReportAfterMove.Tickets[0].Labels, expectedTicketLabels) {
