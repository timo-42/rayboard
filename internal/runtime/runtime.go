...
func runCombined(ctx context.Context, cfg config.Config, stdout, stderr io.Writer) error {
	...
	hookService := tracker.NewHookService(db.SQL, authorizer, tracker.WithHookOpenRouterService(openRouterService), tracker.WithHookRunStore(runStore))
	...
	createPageService := tracker.NewCreatePageService(db.SQL, trackerService, authorizer, tracker.WithCreatePageOpenRouterService(openRouterService), tracker.WithCreatePageRunStore(runStore))
