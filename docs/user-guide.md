...
- workflow status replacement, board create/select/edit/delete with board definition metadata summaries and a selected-board column settings overview, board ticket summary metrics, saved-view board filters, advisory board WIP limits with per-column count warnings, board-backed ticket columns, backlog summary metrics, backlog sprint workload summaries, backlog sprint assignment and drag/drop reorder, and board card drag/drop status changes for the selected project;
