...
- a settings page under `/settings` for global attachment/webhook/demo settings, OpenRouter provider management, Shoutrrr notification destination management and test-send, notification policy CRUD with scoped destination-name selection, project notification defaults, notification delivery history/manual retry with loaded-delivery health summaries, notification hook CRUD plus policy-aware preview/routing inspection, security audit-log inspection when permitted, and personal notification preferences for every signed-in user;
...
- a custom-fields panel for listing, creating, updating, and deleting project fields with metadata summaries, plus field-aware ticket create/card controls for typed custom-field values;
...
- compact search with text/CEL filters, field-aware custom-field filter building, cursor-paginated results, saved-view list pagination, metadata summaries, create, edit, apply, and delete controls for query, sort, columns, display mode, grouping, and pinning, plus pinned project saved views in project navigation and saved-view filters in project boards;
...
- compact sprint reports with live current-assignment and completed snapshot scope markers, sprint health summaries from sprint dates/state, story-point velocity when estimates exist, ticket-count fallback analytics, remaining count, burnup summary, and ticket links for the selected project.
- richer version release reports with live current-assignment and released snapshot scope markers, release health summaries from target/release dates, progress percentage, story-point totals when estimates exist, total/done/open counts, status breakdowns, component breakdowns, unassigned-component highlighting, and ticket links for the selected version.

Token secrets are shown only when created and are not listed later.

It does not currently expose all backend endpoints.

Richer board settings beyond inline edits, definition metadata summaries, summary metrics, saved-view filters, and WIP warnings, richer backlog planning beyond summary metrics, sprint workload summaries, sprint assignment, reorder controls, and drag/drop, richer sprint reporting beyond compact selected-sprint summaries, health summaries, and point/ticket-count analytics, roadmap capacity planning beyond aggregate insights and read-only monthly summary buckets, richer custom-field layout screens beyond metadata summaries, ticket controls, and search filter building, richer saved-view UI beyond metadata summaries, pinned project navigation, and board filters, richer notification delivery analytics beyond loaded Settings summaries and policy-aware routing previews, richer automation run-history analytics beyond loaded status summaries, completion/failure rates, state filters, duration metrics, oldest/newest run timestamps, trigger-type breakdowns, and raw cron, webhook, ticket-hook, and create-page rows, and advanced release planning beyond version drilldowns and release health summaries are **Planned**.
