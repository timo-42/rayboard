...
- a backlog panel with planning summary metrics, estimate coverage, status breakdowns, priority breakdowns, label breakdowns, component breakdowns, assignee breakdowns, sprint workload summaries, start-date breakdowns, due-date breakdowns, readiness summaries, risk summaries, attention summaries, ticket-age breakdowns, update-freshness breakdowns, current sprint labels, sprint assignment/removal controls, up/down controls, and native drag/drop ticket reordering through `/api/projects/{project_id}/backlog`;
