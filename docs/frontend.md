...
- a backlog panel with planning summary metrics, sprint workload summaries, current sprint labels, sprint assignment/removal controls, up/down controls, and native drag/drop ticket reordering through `/api/projects/{project_id}/backlog`;
...
- a roadmap panel with a scheduled epic timeline, aggregate capacity insights, read-only monthly capacity summary buckets, drag/drop timeline rescheduling, quick scheduling for unscheduled epics, inline schedule edits, child-ticket progress, and dependency list/editing controls, plus ticket-form fields for epics, parent epics, and roadmap dates;
- a custom-fields panel for listing, creating, updating, and deleting project fields with metadata summaries and a layout overview, plus field-aware ticket create/card controls for typed custom-field values;
...
- richer version release reports with live current-assignment and released snapshot scope markers, release health summaries from target/release dates, release timeline summaries, scope-change summaries, progress percentage, story-point totals when estimates exist, total/done/open counts, status breakdowns, component breakdowns, unassigned-component highlighting, and ticket links for the selected version.

Token secrets are shown only when created and are not listed later.

It does not currently expose all backend endpoints.

Richer board settings beyond inline edits, definition metadata summaries, the selected-board column settings overview, summary metrics, saved-view filters, and WIP warnings, richer backlog planning beyond summary metrics, sprint workload summaries, sprint assignment, reorder controls, and drag/drop, richer sprint reporting beyond compact selected-sprint summaries, health summaries, point/ticket-count analytics, scope-change summaries, and assignee workload summaries, roadmap capacity planning beyond aggregate insights and read-only monthly summary buckets, richer custom-field layout screens beyond metadata summaries, layout overview, ticket controls, and search filter building, richer saved-view UI beyond metadata summaries, list overview scope breakdowns, pinned project navigation, and board filters, richer notification delivery analytics beyond loaded Settings health/timing summaries and policy-aware routing previews, richer automation run-history analytics beyond loaded status summaries, completion/failure rates, state filters, duration metrics, oldest/newest run timestamps, trigger-type breakdowns, and raw cron, webhook, ticket-hook, and create-page rows, and advanced release planning beyond version drilldowns, release health summaries, release timeline summaries, and scope-change summaries are **Planned**.
