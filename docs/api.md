...
Sprint progress includes ticket counts plus `story_points_total`, `story_points_done`, `story_points_remaining`, and `story_points_unestimated`, and estimated ticket coverage is derived as `total - story_points_unestimated`. The embedded sprint report also summarizes ticket readiness gaps for missing assignees, estimates, start dates, and due dates, plus risk counts for open overdue, stale, unassigned, and unscheduled tickets. Sprint analytics use ticket counts when no report ticket has an estimate, returning velocity with `unit: "tickets"`. When at least one report ticket has `story_points`, burndown, burnup, and velocity use point values, and unestimated tickets contribute zero points.
