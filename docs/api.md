...
The custom-field API supports project-scoped field definitions, select options, typed ticket values, server-side validation during ticket create/update, and CEL search filters through `custom.<field_key>`. The embedded browser UI exposes basic field list/create/update/delete controls, layout overview type and requirement insight counts, usage coverage summaries, select option usage summaries across current project tickets, JSON ticket custom-field value editing, and field-aware `/search` controls that append safe custom-field CEL clauses to the search filter. Richer field schemas and richer field-aware layout screens beyond these summaries are **Planned**.
