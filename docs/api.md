...
Swagger UI and the OpenAPI security schemes expose only bearer API tokens in the authorization dialog. Cookie-authenticated browser requests are still accepted at runtime; when no bearer `Authorization` header is present, the embedded Swagger UI sends browser cookies and automatically copies `rayboard_csrf` into `X-CSRF-Token`. When a bearer token is present, Swagger UI omits browser credentials and clears the CSRF header. Bearer-token requests do not need CSRF.
...
The board/workflow API defines ordered project workflow statuses and board definitions whose ordered columns map to those status slugs. Board definitions can include optional per-column WIP limits keyed by status slug. The embedded project page exposes status replacement, board creation/selection/edit/delete, board-backed ticket columns, per-column count/limit warnings, compact flow-balance summaries, selected-board priority, due-date, issue-type, label, component, version, parent epic, and sprint breakdowns, selected-board assignee workload and reporter breakdown summaries, selected-board estimate coverage, compact capacity utilization overviews, selected-board risk summaries for blocked/overdue/stale/high-priority work, and browser drag/drop movement between board columns. Richer board planning beyond advisory WIP warnings, flow-balance summaries, priority, due-date, issue-type, label, component, version, parent epic, and sprint breakdowns, assignee workload and reporter summaries, estimate coverage, capacity utilization summaries, and risk summaries is **Planned**.
...
The sprint API supports sprint CRUD within a project, starting and completing sprints, sprint reports with ticket-count or story-point analytics, and assigning or removing a ticket from a sprint. The embedded browser UI exposes sprint list/create/edit/state-filter/start/complete/delete, ticket-card and backlog sprint assignment/removal, story point editing, backlog start-date/due-date/readiness/risk/attention/label/component/version/parent-epic summaries, and compact sprint reports with status, start-date, due-date, priority, issue-type, label, estimate-coverage, component, version, parent epic, and reporter breakdowns for the selected project. Richer backlog planning beyond assignment controls and frontend-derived summaries, and sprint reporting beyond compact analytics are **Planned**.
...
Sprint report responses use `spec.sprint` for the sprint resource and `status.scope`, `status.snapshot_at`, `status.progress`, `status.analytics`, and `status.tickets` for computed report data. Planned and active sprint reports use scope `current` and read the current ticket assignments. Completed sprint reports use scope `completed_snapshot` and read a committed ticket membership snapshot captured in the same transaction that completed the sprint. Snapshot reports keep membership stable when tickets are later moved out of the sprint; ticket fields such as title, status, type, reporter, assignee, priority, parent epic, labels, start date, due date, creation time, update time, component, version, and story points are still read from the current ticket records, and deleted tickets are omitted.

Sprint progress includes ticket counts plus `story_points_total`, `story_points_done`, `story_points_remaining`, and `story_points_unestimated`, and estimated ticket coverage is derived as `total - story_points_unestimated`. The embedded sprint report also summarizes ticket readiness gaps for missing assignees, estimates, start dates, and due dates, risk counts for open overdue, stale, unassigned, and unscheduled tickets, and attention counts for blocked-like, high-priority, unestimated high-priority, and stale high-priority open work. Sprint analytics use ticket counts when no report ticket has an estimate, returning velocity with `unit: "tickets"`. When at least one report ticket has `story_points`, burndown, burnup, and velocity use point values, and unestimated tickets contribute zero points.

## Components and Versions

The components/versions API supports project component CRUD, project version/release CRUD, current and released-snapshot version reports, and assignment of tickets to a component or version through ticket create/update fields. The embedded browser UI exposes basic component/version list/create/update/delete, component ownership coverage summaries, per-component owner/default-assignee metadata, component visible-ticket status summaries, version lifecycle summaries, version target/release date coverage summaries, version visible-ticket status, priority, issue-type, label, assignee, reporter, due-date, start-date, and age summaries, version release/archive state changes, project-level version target-date health and release timing variance summaries, ticket component/version assignment, project ticket filtering by component or version, and richer release reports for the selected version with release analytics, status breakdowns, start-date breakdowns, due-date breakdowns, ticket-age breakdowns, update freshness breakdowns, readiness summaries, risk summaries, attention summaries, priority breakdowns, issue-type breakdowns, label breakdowns, component breakdowns, parent epic breakdowns, sprint breakdowns, assignee workload summaries, reporter breakdowns, estimate coverage, point totals, and unassigned-component highlighting. Project boards also expose selected-board attention summaries for blocked-like, high-priority, unestimated high-priority, and stale high-priority open work. Roadmap timeline screens and advanced release planning UI beyond version drilldowns, target-date health summaries, and release timing variance summaries are **Planned**.
...
Version report responses use `spec.version` for the version resource and `status.scope`, `status.snapshot_at`, `status.progress`, `status.analytics`, and `status.tickets` for computed report data. Planned and archived version reports use scope `current` and read current non-deleted ticket assignments. When a version transitions to `released`, Rayboard captures the current non-deleted assigned ticket IDs in a release snapshot; released version reports use scope `released_snapshot` and keep that membership stable if tickets are later moved to another version or cleared. Deleted tickets are still omitted from report output. Ticket fields such as title, status, priority, type, assignee, component, and story points are read from current ticket records.

Version progress includes `total`, `done`, `open`, `unassigned_component`, `by_status`, `story_points_total`, `story_points_done`, `story_points_remaining`, and `story_points_unestimated`; `done` counts tickets whose current status is `done`, `open` is `total - done`, and estimated ticket coverage is derived as `total - story_points_unestimated`. Version analytics include `burndown`, `burnup`, and `velocity` with ticket-count units when no scoped ticket has story points and point units when estimates exist.
...
The roadmap API lists epics, direct child-ticket progress, persisted monthly capacity targets, and dependency edges from existing ticket links. Epics are regular tickets with `type: "epic"`, optional `start_date` and `due_date`, and direct child tickets linked by `parent_ticket_id`. The embedded browser UI exposes a scheduled epic timeline, read-only monthly capacity summary buckets derived from roadmap dates and unfiltered project child tickets, selected-month capacity drilldowns for the contributing epics, persisted monthly point target indicators for under/over target buckets, draggable timeline rescheduling that preserves epic duration, quick scheduling for unscheduled epics, inline epic schedule editing, child progress, ticket-form fields for epic creation, parent epic assignment, roadmap dates, dependency create/delete controls backed by ticket-link APIs, and a read-only dependency graph for roadmap-scoped links. Richer roadmap capacity planning beyond persisted month targets, read-only buckets, and drilldowns is **Planned**.

| Method | Path | Body or Query |
| --- | --- | --- |
| `GET` | `/api/projects/{project_id}/roadmap` | none |
| `GET` | `/api/projects/{project_id}/roadmap/capacity-targets` | none |
| `PUT` | `/api/projects/{project_id}/roadmap/capacity-targets/{month}` | `{"spec":{"target_points":24.5}}` |
...
| `POST` | `/api/ticket-create-pages/{page_id}/preview` | Executes saved Lua/AI form logic for project automation managers, records a run, and returns the effective renderable schema without publishing or submitting a ticket. |
| `GET` | `/api/ticket-create-pages/{page_id}/runs` | Lists saved preview run history for project automation managers. |
...
Create-page responses use `metadata` for page/project/owner/timestamps, `spec` for the page definition, and `status.deleted_at` for soft-delete state. Schema and preview responses use `metadata.page_id`, `metadata.project_id`, and `metadata.slug`, return the renderable page definition in `spec`, and expose `status.enabled`. Disabled pages are not resolvable by slug, but managers can preview them by ID before enabling them. Saved previews write shared automation run records with trigger type `ticket_create_page`, the page ID as `trigger_ref`, engine type without script/prompt source, renderable output, status, error, and timestamps. Submit responses return the normal ticket resource shape.

When `spec.form_lua_script` is set, schema resolution and submission run that Lua script in the shared JSON sandbox with `context`, `page`, `json`, `rayboard.json`, and `rayboard.log(message)`. The script may return a table containing `field_layout`, `defaults`, and/or `description`; returned raw HTML fields are rejected. Layout items may include nested `fields` arrays for grouped controls, and browser rendering uses text-only DOM nodes for headings/help copy. The script does not receive filesystem, shell, network, database, or Rayboard API helpers.
...
Delivery resources use `metadata` for queue identity, scope, policy snapshot, and destination snapshot; `spec` for event/message payload and retry budget; and `status` for current state, attempt counts, timestamps, and last error. The embedded Settings page summarizes the currently loaded delivery rows with counts by state, delivered percentage, retryable failed/canceled count, oldest/newest timestamps, latest failure detail, event-type and destination breakdowns, and retry-pressure/exhausted-attempt counts.
...
Preview requests use `{"spec":{"event_type":"ticket_assigned","message":"Assigned AUTO-1","payload":{"ticket_id":"ticket_123"},"destination_ids":["dest_123"]}}`. Hook run resources use `metadata` for run identity, `spec` for trigger/input context, and `status` for state, output, error, and timestamps. The embedded Settings page supports scoped destination-name selection for policies, policy-seeded hook previews, preview output with destination names and suppression state, compact routing summaries for suppression/destination/message/payload changes, and loaded-row delivery analytics by event type, destination, and retry pressure. Dashboard/view notification delivery analytics beyond the Settings summary and richer recipient rules are **Planned**.

## Webhooks

The webhook slice implements project-scoped incoming and outgoing webhook definitions. Incoming webhooks have hashed bearer tokens, one-time token display, token rotation, Lua or AI execution for authenticated incoming requests, constrained Rayboard action helpers, and shared automation run history. Outgoing webhook definitions can be created, listed, updated, and deleted with `spec.event_types`, and the backend has a durable queued-delivery model for matching domain events. Due outgoing deliveries can be shaped by Lua or AI, sent through a controlled HTTP client to the configured outgoing webhook base URL, retried with backoff, inspected through the API, and manually requeued after failure. The embedded `/automation` page provides project webhook management with inline editing, token rotation, run history, and outgoing delivery inspection.
...
Project ticket hooks are project-scoped automation resources that run during ticket create/update. Before hooks can validate or transform the pending ticket payload; after hooks run after commit and cannot roll back committed changes. The `/automation` browser UI provides project ticket-hook list, create, inline edit, delete, enable/disable, preview, and recent run-history controls; the API remains the source of truth for the full resource shape.
...
| `GET` | `/api/ticket-hooks/{hook_id}/runs` | Lists persisted real execution history for the saved hook. Preview calls are not persisted. |
| `DELETE` | `/api/ticket-hooks/{hook_id}` | Soft-deletes and disables the hook. |

Ticket hook responses use `metadata` for IDs/project/timestamps, `spec` for event, phase, order, enabled state, and engine configuration, and `status.last_error` for the last persisted execution error. Real hook executions also write shared automation run records with trigger type `ticket_hook`, the hook ID as `trigger_ref`, engine type without script/prompt source, ticket input/output/logs, status, error, and timestamps. Preview responses use `metadata.hook_id`, echo the preview input in `spec`, and return `status.output`, transformed `status.ticket` when present, `status.logs`, and `status.error` for reject/runtime errors. The OpenAPI schema represents `spec.engine` as a discriminated `oneOf` object. Lua hooks require `engine.script`; AI hooks require `engine.prompt` and `engine.provider_id`. AI hook prompts receive hook context and ticket input, must return a JSON object, and use the same top-level `ticket` transform and `reject` response contract as Lua hooks. AI hooks call OpenRouter through the configured provider without exposing API keys.
...
| Method | Path | Body or Query |
| --- | --- | --- |
| `POST` | `/api/engines/test` | `{"spec":{"surface":"scratch","project_id":"","actor_user_id":"","engine":{"type":"lua","script":"return { ok = true, input = input }"},"context":{},"input":{"title":"Preview"},"dry_run":true,"validate_only":false}}` |
...
The current endpoint is dry-run and mutation-free; `spec.dry_run` is normalized to `true`, and missing `spec.surface` defaults to `scratch`. When `spec.validate_only=true`, Rayboard validates actor/project permissions, surface, engine discriminator fields, AI provider availability, and WASM module shape without executing Lua, calling OpenRouter, or instantiating WASM. Lua receives `context`, `input`, `json`, `rayboard.json`, and `rayboard.log(message)` only during execution, but it does not receive ticket/comment/search helper functions. `spec.context` is merged with normalized `surface`, `project_id`, `actor_user_id`, `dry_run`, and `validate_only` values before execution. AI tests append the supplied context/input to the prompt and call the selected OpenRouter provider in JSON-object mode unless `validate_only` is set. WASM tests use `engine.type = "wasm"` with `engine.module_base64`, a base64-encoded WASI command module. Rayboard writes `surface`, `context`, `input`, `dry_run`, and `validate_only` JSON to stdin; the module must write one JSON object to stdout. Stderr lines are captured as logs. No filesystem mounts, environment variables, shell, raw sockets, direct SQLite handles, or Rayboard secrets are exposed.

Responses use `metadata`, `spec`, and `status`. The response redacts `spec.engine.script`, `spec.engine.prompt`, and `spec.engine.module_base64`; run history stores the engine type, actor, surface, normalized context, dry-run and validate-only flags, and supplied input, not raw source, prompt, or module bytes. `status.mode` is `validated` for validate-only checks, `previewed` when the engine returns dry-run action previews, and `executed` for ordinary engine runs. `status.output` contains validation metadata or the validated engine output, `status.action_previews` contains non-persistent dry-run action preview objects returned by the engine, `status.logs` contains captured log lines, `status.duration_millis` reports elapsed execution time when available, and `status.engine` contains redacted engine metadata. Runtime failures return a normal resource response with `status.state = "failed"` and `status.error`.
...
Cron job CRUD and manual runs require automation management permissions. Run history uses the shared automation run-history model and must not expose secrets. Run resources use `metadata` for run identity/timestamps, `spec` for trigger and input context, and `status` for state, output, error, and start/finish timestamps. The embedded automation UI summarizes loaded run histories with status counts, completion/failure rates, duration metrics, oldest/newest timestamps, trigger-type breakdowns, and failure-cause breakdowns. Lua cron jobs execute scripts with constrained Rayboard helpers. AI cron jobs call the configured OpenRouter provider, require JSON-object output, and store the validated object plus model/usage metadata in run history without exposing prompts or API keys. AI cron output may include an `actions` array with action types `search`, `get_ticket`, `create_ticket`, `update_ticket`, and `comment`; actions are capped at 20, run as the cron job owner through normal backend service/RBAC paths, and record results under `output.action_results`.
...
Implemented cron Lua helpers are `rayboard.log`, `rayboard.search`, `rayboard.get_ticket`, `rayboard.create_ticket`, `rayboard.update_ticket`, and `rayboard.comment`. Helpers execute through normal backend service/RBAC paths as the cron job owner. AI cron actions expose the same constrained action set as JSON objects returned by OpenRouter. Incoming webhook Lua/AI exposes the same constrained action helper set as the webhook actor. Outgoing webhook Lua/AI shapes controlled outbound HTTP requests from domain-event context. Notification hooks use Lua/AI to suppress, transform, or narrow destination routing for notification plans before delivery rows are enqueued, with saved-hook preview and run history APIs. The backend ticket-hook runner and preview API expose `context`, `ticket`, optional `current`, and `rayboard.log`. Custom create-page Lua exposes `context`, `page`, JSON helpers, and `rayboard.log` for structured schema/default transformation. Custom create-page AI uses OpenRouter to return the same validated structured schema/default output.
