...
| `POST` | `/api/ticket-create-pages/{page_id}/preview` | Executes saved Lua/AI form logic for project automation managers and returns the effective renderable schema without publishing or submitting a ticket. |
...
Create-page responses use `metadata` for page/project/owner/timestamps, `spec` for the page definition, and `status.deleted_at` for soft-delete state. Schema and preview responses use `metadata.page_id`, `metadata.project_id`, and `metadata.slug`, return the renderable page definition in `spec`, and expose `status.enabled`. Disabled pages are not resolvable by slug, but managers can preview them by ID before enabling them. Submit responses return the normal ticket resource shape.
